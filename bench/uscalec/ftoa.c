...
void FixedWidth(double f, int n, uint64_t *dp, int *pp) {
	...
	int p = n - 1 - log10Pow2(e+63);
	//printf("p = %d\n", p);
	unrounded u = uscale(m, prescale(e, p, log2Pow10(p)));
	//printf("u = %#llx\n", u);
	uint64_t d = uround(u);
	//printf("d = %lld\n", d);
	if(d >= uint64pow10[n]) {
...
static const char smalls[] = "00010203040506070809"
...
formatBase10(char *dst, uint64_t d64, int nd)
{
	while ((d64>>32) != 0) {
		...
		uint32_t y1 = (y / 100) * 2;
		uint32_t y0 = (y % 100) * 2;
		memmove(dst+nd-8, smalls+x1, 2);
		memmove(dst+nd-6, smalls+x0, 2);
		memmove(dst+nd-4, smalls+y1, 2);
		memmove(dst+nd-2, smalls+y0, 2);
		nd -= 8;
	}
	...
	while(d >= 10000) {
		...
		uint32_t x1 = (x / 100) * 2;
		uint32_t x0 = (x % 100) * 2;
		memmove(dst+nd-4, smalls+x1, 2);
		memmove(dst+nd-2, smalls+x0, 2);
		nd -= 4;
	}
	if(d >= 100) {
		uint32_t x = d % 100;
		d /= 100;
		memmove(dst+nd-2, smalls+2*x, 2);
		nd -= 2;
	}
	if(d >= 10) {
		memmove(dst+nd-2, smalls+2*d, 2);
		return;
	}
	dst[nd-1] = '0' + d;
}
...
void Format(char *dst, uint64_t d, int p, int nd) {
	...
	if (p < 100) {
		memmove(dst+n+2, smalls+2*p, 2);
		dst[n+4] = 0;
		return;
	}
	dst[n+2] = '0' + p/100;
	memmove(dst+n+3, smalls+2*(p%100), 2);
	dst[n+5] = 0;
	return;
}
